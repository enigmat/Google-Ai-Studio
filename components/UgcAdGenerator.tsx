...
interface UgcAdGeneratorProps {
  onSubmit: (productImageUrl: string, productName: string, productDescription: string) => void;
  isLoading: boolean;
}

const UgcAdGenerator: React.FC<UgcAdGeneratorProps> = ({ onSubmit, isLoading }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError(null);
    onSubmit(productImageUrl, productName, productDescription);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        >
          {productImageUrl ? (
            ...
            <div className="text-center text-gray-500">
              ...
              <p className="text-xs">Your product image here</p>
            </div>
          )}
        </label>
        <input
      ...
      <textarea
        ...
        placeholder="Product Description (e.g., 'A hydrating facial serum with vitamin C and hyaluronic acid...')"
      ...
      <button
        type="submit"
        disabled={isLoading || !productImageUrl || !productName.trim() || !productDescription.trim()}
