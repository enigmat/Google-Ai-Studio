...
const ImageDisplay: React.FC<ImageDisplayProps> = ({ 
    ...
    // FIX: Update default prop handlers to accept an argument, resolving type conflicts.
    onDownloadClick = (_) => {}, onEditClick = (_) => {}, onRemoveObjectClick = (_) => {}, onRemoveBackground = (_) => {}, onUpscale = (_) => {}, onAnimateClick = (_) => {}, onExpandClick = (_) => {},
    onGetPrompt = (_) => {}, onSaveToAirtable = (_) => {}, airtableConfigured = false, savingToAirtableState = { status: 'idle', imageId: null },
    hideActions = false,
  ...
  const getGridColsClass = (count: number) => {
    ...
    if (count > 4) return 'grid-cols-4'; // For 8 and 12 variations
    return 'grid-cols-2';
  }
  ...
  const isGrid = !isLoading && imagesData && imagesData.length > 1;

  return (
    <div 
      className={`relative w-full bg-gray-800 border-2 border-dashed border-gray-700 rounded-lg p-4 transition-all duration-300 ${!isGrid ? 'flex items-center justify-center' : ''}`}
      style={!isGrid ? { aspectRatio: aspectRatio.replace(':', ' / ') } : {}}
    >
      ...
      {isLoading ? (
        ...
        <div className={isGrid ? `w-full grid ${getGridColsClass(imagesData.length)} gap-4` : 'w-full h-full'}>
          {imagesData.map((image, index) => {
            ...
            return (
              <div 
                ...
                style={isGrid ? { aspectRatio: aspectRatio.replace(':', ' / ') } : { height: '100%' }}
              >
                <img
                  src={image.url}
                  alt={image.title || `Generated by AI ${index + 1}`}
                  className={`w-full h-full rounded-md ${isGrid ? 'object-cover' : 'object-contain'}`}
                />
                {!hideActions && (
                  <>
